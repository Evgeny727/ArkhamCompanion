...
fun CardsFiltersActionsScreen(
    ...
    val filteredActions = remember(actionCodes, resources, searchQuery) {
        val query = searchQuery.trim()

        actionCodes
            .map { code ->
                code to resources.getString(getLocalizedAction(code))
            }
            .filter { (_, name) ->
    ...
    Column(
        ...
        ArkhamSearchBox(
            ...
            searchPlaceholder = stringResource(R.string.search_for_actions)
        )

        AnimatedVisibility(actionCodes.isEmpty()) {
            Column(
        ...
        LazyColumn(
            ...
            items(
                ...
                ArkhamCheckboxButton(
                    title = text,
                    isSelected = action in selectedActions,
